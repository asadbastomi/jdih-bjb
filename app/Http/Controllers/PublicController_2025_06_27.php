...
class PublicController extends Controller
{
    ...
    function orderChartPieData($data)
    {
        $desiredOrder = [
            'Peraturan Daerah',
            'Peraturan Wali Kota',
    ...
    {
        // Cari data buku dari tabel regulasi
        $this->data['data'] = Regulasi::select('regulasi.*', 'kategori.nama_singkat', 'kategori.nama')
            ->where('regulasi.id', $id)
            ->leftJoin('kategori', 'regulasi.kategori_id', '=', 'kategori.id')
            ->whereHas('kategori', function($query) {
        ...
        // Cek hit/view count
        $this->data['hit'] = PopularItem::where('id_item', $id)->where('id_kategori', $this->data['data']->kategori_id)->first()->hit ?? 0;
        $this->data['unduhan'] = PopularItem::where('id_item', $id)->where('id_kategori', $this->data['data']->kategori_id)->first()->downloaded ?? 0;

        // Data untuk relasi ubah-cabut
        $regUbahCabut = Regulasi::withUbahCabut();
        $regUbahCabut->where('id_reg_1', $this->data['data']->id);
        $cekperrow = $regUbahCabut->get();
        $regUbahCabutArr = [];
        ...
        // Update hit counter jika diperlukan
        $hitRequest = new Request();
        $hitRequest->merge([
            'id' => $this->data['data']->id,
            'kategori' => $this->data['data']->kategori_id,
