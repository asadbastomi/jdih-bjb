...
function makeButtonLoading() {
    $("form.async [type=submit]").each(function () {
        ...
        window.window[idbtn] = Ladda.create(
            document.querySelector("#" + idbtn)
        );
    });
}
...
function makeForm(formname, method = "post") {
    ...
    $("#" + formname + " .send").each(function () {
        ...
        if (
            ...
            var valdata = $("#" + $(this).attr("id"))?.select2?.("val") || $(this).val();
        ...
        success: function (response) {
            ...
            if (actSuccess) {
                if (actSuccess.request == "redirect") {
                    setTimeout(function () {
                        window.location = actSuccess.url;
                    }, actSuccess.after);
                }
                ...
                if (actSuccess.request == "removefromtable") {
                    $(
                        "#" +
                            actSuccess.table +
                            ' tbody tr td div.data-store[data-id="' +
                            response.data.id +
                            '"]'
                    )
                        .parent()
                        .parent()
                        .addClass("readytodelete delete");
                    crrentPage = $(
                ...
                if (actSuccess.request == "surveyThanks") {
                    $("#surveyThanks").removeClass("d-none");
                    $("#surveyBody").addClass("d-none");
                    $("#surveyFooter").addClass("d-none");

                    // Reset flag after survey is completed
                    sessionStorage.removeItem("visitedLegalProduct");

                    // Set flag after survey is completed
