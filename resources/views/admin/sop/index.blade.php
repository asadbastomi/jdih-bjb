@extends('layouts.detached', ['title' => 'Sop'])
                ...
                <form id="formsop" class="async">
                    ...
                    <div class="modal-body p-4">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="control-label" required>Nama SOP</label>
                                    <input type="text" class="form-control send" id="nama" placeholder="Nama SOP" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="control-label">Deskripsi SOP (Opsional)</label>
                                    <input type="text" class="form-control send" id="deskripsi" placeholder="Deskripsi SOP">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr"
                            data-style="zoom-in" id="btnsop"><i class="mdi mdi-content-save mr-2"></i>
                            Save changes</button>
                    </div>
                </form>
            </div>
...
@section('script')
    ...
    <script>
        ...
        $('.dropify').dropify();
        $(function ($) {
            loadTable('{{ route('api.sop.fetch') }}', textserach);
                    ...
                    if (isnew.status) {
                        option = {
                            ...
                            'success': {
                                ...
                                'fetch': '{{ route('api.sop.fetch') }}',
                                'after': 300
                            }
                        }
                        sentData('{{ route('api.sop.store') }}', option);
                    } else {
                        option = {
                            'module': 'sop',
                            'method': 'put',
                        ...
                        sentData('{{ url('api/sop') }}/' + isnew.id, option);
                    }
                }
            });
            ...
            $(document).on('click', '.btneditdata', function (event) {
                ...
                option = {
                    'success': {
                        ...
                        'modal': 'modalform',
                        'form': 'formsop',
                        'field': ['nama', 'deskripsi', 'file_sop'],
                    }
                }
                getData('{{ url('api/sop') }}/' + rowid + '/edit', option);
            });
            $(document).on('click', '.btndeletedata', function (event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        sentData('{{ url('api/sop') }}/' + rowid, option);
                    }
                });
            });
            $(document).on('click', '.page-link', function (event) {
                ...
                loadTable('{{ route('api.sop.fetch') }}', textserach, page);
            });
