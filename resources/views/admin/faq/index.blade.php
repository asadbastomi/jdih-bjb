@extends('layouts.detached', ['title' => 'FAQ'])
                ...
                <form id="formfaq" class="async">
                        ...
                        {{-- <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control send" id="jawaban" placeholder="Jawaban">
                                </div>
                            </div>
                        </div> --}}
                        <div class="row">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr"
                            data-style="zoom-in" id="btnfaq"><i class="mdi mdi-content-save mr-2"></i>
                            Save changes</button>
                    </div>
                </form>
            </div>
...
@section('script')
    ...
    <script>
        var textserach = '';
        $(function ($) {
            loadTable('{{ route('api.faq.fetch') }}', textserach);
                    ...
                    if (isnew.status) {
                        option = {
                            ...
                            'success': {
                                ...
                                'fetch': '{{ route('api.faq.fetch') }}',
                                'after': 300
                            }
                        }
                        sentData('{{ route('api.faq.store') }}', option);
                    } else {
                        option = {
                            'module': 'faq',
                            'method': 'put',
                        ...
                        sentData('{{ url('api/faq') }}/' + isnew.id, option);
                    }
                }
            });
            ...
            $(document).on('click', '.btneditdata', function (event) {
                ...
                option = {
                    'success': {
                        ...
                        'modal': 'modalform',
                        'form': 'formfaq',
                        'field': ['pertanyaan', 'jawaban'],
                    }
                }
                getData('{{ url('api/faq') }}/' + rowid + '/edit', option);
            });
            $(document).on('click', '.btndeletedata', function (event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        sentData('{{ url('api/faq') }}/' + rowid, option);
                    }
                });
            });
            $(document).on('click', '.page-link', function (event) {
                ...
                loadTable('{{ route('api.faq.fetch') }}', textserach, page);
            });
