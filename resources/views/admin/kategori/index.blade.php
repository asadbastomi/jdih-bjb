...
@extends('layouts.detached', ['title' => 'Categories'])
    ...
    <div id="modalform" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form id="formkategori" class="async" >
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control send" id="nama" placeholder="Nama Kategori">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control send" id="nama_singkat" placeholder="Nama Singkat">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr" data-style="zoom-in" id="btnkategori"><i class="mdi mdi-content-save mr-2"></i> Save changes</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('script')
    ...
    var textserach = '';
    $(function($){
        loadTable('{{ route('api.kategori.fetch') }}', textserach);

        $(document).on('submit','form.async',function(){
            event.preventDefault();
            // Form Role
            if ($(this).attr('id')=='formkategori') {
                isnew = isNew('formkategori');
                if (isnew.status) {
                    option = {
                        'module' : 'kategori',
                        'success' : {
                            ...
                            'fetch' : '{{ route('api.kategori.fetch') }}',
                            'after' : 300
                        }
                    }
                    sentData('{{ route('api.kategori.store') }}', option);
                } else {
                    option = {
                        'module' : 'kategori',
                        'method' : 'put',
                    ...
                    sentData('{{ url('api/kategori') }}/'+isnew.id, option);
                }
            }
        });
        ...
        $(document).on('click', '.btneditdata', function(event) {
            ...
            option = {
                'success' : {
                    ...
                    'modal' : 'modalform',
                    'form' : 'formkategori',
                    'field' : ['nama', 'nama_singkat'],
                }
            }
            getData('{{ url('api/kategori') }}/'+rowid+'/edit', option);
        });
        $(document).on('click', '.btndeletedata', function(event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    sentData('{{ url('api/kategori') }}/'+rowid, option);
                }
            });
        });
        $(document).on('click', '.page-link', function(event) {
            ...
            loadTable('{{ route('api.kategori.fetch') }}', textserach, page);
        });
