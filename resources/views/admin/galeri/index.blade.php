@extends('layouts.detached', ['title' => 'Galeri'])
                ...
                <form id="formgaleri" class="async">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr"
                            data-style="zoom-in" id="btngaleri"><i class="mdi mdi-content-save mr-2"></i>
                            Save changes</button>
                    </div>
                </form>
            </div>
...
@section('script')
    ...
    <script>
        ...
        $('.dropify').dropify();
        $(function ($) {
            loadTable('{{ route('api.galeri.fetch') }}', textserach);
            ...
            $(document).on('submit', 'form.async', function () {
                ...
                // Form Role
                if ($(this).attr('id') == 'formgaleri') {
                    isnew = isNew('formgaleri');
                    if (isnew.status) {
                        option = {
                            'module': 'galeri',
                            'success': {
                                ...
                                'fetch': '{{ route('api.galeri.fetch') }}',
                                'after': 300
                            }
                        }
                        sentData('{{ route('api.galeri.store') }}', option);
                    } else {
                        option = {
                            'module': 'galeri',
                            'method': 'put',
                        ...
                        sentData('{{ url('api/galeri') }}/' + isnew.id, option);
                    }
                }
            });
            ...
            $(document).on('click', '.btneditdata', function (event) {
                ...
                option = {
                    'success': {
                        ...
                        'modal': 'modalform',
                        'form': 'formgaleri',
                        'field': ['nama_kegiatan', 'foto_kegiatan'],
                    }
                }
                getData('{{ url('api/galeri') }}/' + rowid + '/edit', option);
            });
            $(document).on('click', '.btndeletedata', function (event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        sentData('{{ url('api/galeri') }}/' + rowid, option);
                    }
                });
            });
            $(document).on('click', '.page-link', function (event) {
                ...
                loadTable('{{ route('api.galeri.fetch') }}', textserach, page);
            });
