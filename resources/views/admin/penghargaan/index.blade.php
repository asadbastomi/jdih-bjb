...
@section('content')
    ...
    @include('admin.penghargaan.modal')
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnAddData').click(function() {
                clearFields();
                $('#modalForm .modal-title').text('Tambah Penghargaan');
        ...
        function loadData() {
            apiRequest('{{ env('JDIH_SVC_PENGHARGAAN_URL') }}', 'GET', {}, function(response) {
                let rows = response.data.map(penghargaan => `
            <tr>
                <td>${penghargaan.nama}</td>
                <td>${penghargaan.detail}</td>
                <td>
                    <img src="${penghargaan.image || 'https://placehold.jp/150x150.png?text=NO+IMAGE'}" class="img-thumbnail" width="100">
                </td>
                <td>
                    <div class="button-list" style="white-space: nowrap;">
                        <button class="btn btn-xs btn-success btnedit" data-id="${penghargaan.id}">
                            <i class="mdi mdi-pencil-outline"></i>
                        </button>
                        <button class="btn btn-xs btn-danger btndelete" data-id="${penghargaan.id}">
        ...
        `).join('');
                $('#penghargaan-list').html(rows);
            });
        }

        function submitForm() {
            let id = $('#penghargaanId').val();
            let formData = new FormData($('#formPenghargaan')[0]);
            if (id) formData.append('_method', 'PUT');
            let url = id ? `{{ env('JDIH_SVC_PENGHARGAAN_URL') }}/${id}` : `{{ env('JDIH_SVC_PENGHARGAAN_URL') }}`;
        ...
        function editData(id) {
            ...
            apiRequest(`{{ env('JDIH_SVC_PENGHARGAAN_URL') }}/${id}`, 'GET', {}, function(response) {
                let data = response.data;
                $('#penghargaanId').val(data.id);
                $('#nama').val(data.nama);
                $('#detail').val(data.detail);
                if (data.file_url) {
                    $('#previewImage').attr('src', data.file_url).show();
                }
                $('#modalForm .modal-title').text('Edit Penghargaan');
                $('#modalForm').modal('show');
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result) {
                    apiRequest(`{{ env('JDIH_SVC_PENGHARGAAN_URL') }}/${id}`, 'DELETE', {}, function() {
