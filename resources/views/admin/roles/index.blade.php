...
@extends('layouts.detached', ['title' => 'Roles'])
            ...
            <div class="modal-content">
                <form id="formrole" class="async" >
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr" data-style="zoom-in" id="btnrole"><i class="mdi mdi-content-save mr-2"></i> Save changes</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('script')
    ...
    var textserach = '';
    $(function($){
        loadTable('{{ route('api.roles.fetch') }}', textserach);

        $(document).on('submit','form.async',function(){
            event.preventDefault();
            // Form Role
            if ($(this).attr('id')=='formrole') {
                isnew = isNew('formrole');
                if (isnew.status) {
                    option = {
                        'module' : 'role',
                        'success' : {
                            ...
                            'fetch' : '{{ route('api.roles.fetch') }}',
                            'after' : 300
                        }
                    }
                    sentData('{{ route('api.roles.store') }}', option);
                } else {
                    option = {
                        'module' : 'role',
                        'method' : 'put',
                    ...
                    sentData('{{ url('api/roles') }}/'+isnew.id, option);
                }
            }
        });
        ...
        $(document).on('click', '.btneditdata', function(event) {
            ...
            getData('{{ url('api/roles') }}/'+rowid+'/edit', option);
        });
        $(document).on('click', '.btndeletedata', function(event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    sentData('{{ url('api/roles') }}/'+rowid, option);
                }
            });
        });
        $(document).on('click', '.page-link', function(event) {
            ...
            loadTable('{{ route('api.roles.fetch') }}', textserach, page);
        });
