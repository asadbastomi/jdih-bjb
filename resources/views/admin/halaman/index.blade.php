@extends('layouts.detached', ['title' => 'Pages'])
                ...
                <form id="formhalaman" class="async">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="ladda-button btn btn-primary waves-effect waves-light" dir="ltr"
                            data-style="zoom-in" id="btnhalaman"><i class="mdi mdi-content-save mr-2"></i> Save
                            changes</button>
                    </div>
                </form>
            </div>
...
@section('script')
    ...
    <script>
        ...
        var galleryInput = `
                ...
                `;
        $(function($) {
            loadTable('{{ route('api.halaman.fetch') }}', textserach);

            $(document).on('submit', 'form.async', function() {
                event.preventDefault();
                // Form Halaman
                if ($(this).attr('id') == 'formhalaman') {
                    isnew = isNew('formhalaman');
                    if (isnew.status) {
                        option = {
                            'module': 'halaman',
                            'success': {
                                ...
                                'fetch': '{{ route('api.halaman.fetch') }}',
                                'after': 300
                            }
                        }
                        sentData('{{ route('api.halaman.store') }}', option);
                    } else {
                        option = {
                            'module': 'halaman',
                            'method': 'put',
                        ...
                        sentData('{{ url('api/halaman') }}/' + isnew.id, option);
                    }
                }
            });
            ...
            $(document).on('click', '.btneditdata', function(event) {
                ...
                option = {
                    'success': {
                        ...
                        'modal': 'modalform',
                        'form': 'formhalaman',
                        'field': ['judul', 'tipe', 'konten'],
                    }
                }
                getData('{{ url('api/halaman') }}/' + rowid + '/edit', option);
            });
            $(document).on('click', '.btndeletedata', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        sentData('{{ url('api/halaman') }}/' + rowid, option);
                    }
                });
            });
            $(document).on('click', '.page-link', function(event) {
                ...
                loadTable('{{ route('api.halaman.fetch') }}', textserach, page);
            });
